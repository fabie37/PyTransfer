...
def recvFile(conn, header):
        ...
        try:
                ...
                print("Removing corrupted file from server to allow reupload.")
                os.remove(filePath)
...
def getFilePath(f=""):
        ...
        return Path(os.path.join(dir_of_file, f))
...
def fileExist(conn, header):
        ...
        if fileName != "server.py":
                ...
                conn.sendall("PROTEC".encode('utf-8'))
                return True
